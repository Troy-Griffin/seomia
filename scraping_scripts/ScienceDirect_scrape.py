...
if __name__ == "__main__":
    ...
    wait.until(
        EC.visibility_of_element_located(
            (
                By.XPATH,
                "//body[@class='bodybg']/div[@class='minHeight']/div[@class='fullPage']/div[@class='pageContent']/div[@class='pageContentInner']/div[@id='accessibleForm']/form[@method='post']/fieldset/div[2]/input[1]",
            )
        )
    ).send_keys(number)
    ...
    wait.until(
        EC.visibility_of_element_located(
            (
                By.XPATH,
                "//body[@class='bodybg']/div[@class='minHeight']/div[@class='fullPage']/div[@class='pageContent']/div[@class='pageContentInner']/div[@id='accessibleForm']/form[@method='post']/fieldset/div[3]/input[1]",
            )
        )
    ).send_keys(number)
    ...
    wait.until(
        EC.visibility_of_element_located(
            (
                By.XPATH,
                "//div[@class='formButtonArea']//div[@class='formButtonArea']//a[@href='#']",
            )
        )
    ).click()
    ...
    wait.until(
        EC.visibility_of_element_located((By.XPATH, "//textarea[@id='qs']"))
    ).send_keys(search)
    ...
    wait.until(
        EC.visibility_of_element_located(
            (
                By.XPATH,
                "//div[@class='BottomLinksGroup']//div[@class='row search-row']//div[@class='col-xs-24 col-sm-20 col-lg-12']//button[@type='submit']",
            )
        )
    ).click()
    ...
    wait.until(
        EC.visibility_of_element_located(
            (
                By.XPATH,
                "//body[@class='js']/div[@id='react-root']/div[@class='sd-flex-container']/div[@class='sd-flex-content']/div[@class='SearchPage']/div[@class='grid']/div[@class='row']/div[@class='col-xs-24']/section/div[@class='Search']/div[@id='main_content']/div[@id='srp-facets']/div[@class='facet-container']/form[@name='filters']/div/div[2]/fieldset[1]/ol[1]/li[2]/div[1]/label[1]/span[1]",
            )
        )
    ).click()
    wait.until(
    ...
    wait.until(
        EC.visibility_of_all_elements_located(
            (By.XPATH, "//div[@class='result-item-content']")
        )
    )
        ...
        while k == 0:
            wait.until(
                EC.visibility_of_element_located(
                    (
                        By.XPATH,
                        "//li[@class='pagination-link next-link']//a[@class='anchor']",
                    )
                )
            )
            next = driver.find_element(
                By.XPATH, "//li[@class='pagination-link next-link']//a[@class='anchor']"
            )
            for article in articles:
                subject = []

                title = article.find_element(
                    By.XPATH, ".//span[@class='anchor-text']/span"
                ).text

                journal = article.find_element(
                    By.XPATH,
                    ".//a[@class='anchor subtype-srctitle-link anchor-default anchor-has-inherit-color']/span/span",
                ).text

                btn = article.find_element(
                    By.XPATH, ".//span[@class='PreviewButton']/button"
                )
                ...
                try:
                    wait.until(
                        EC.visibility_of_element_located(
                            (
                                By.XPATH,
                                ".//div[@class='abstract-section u-font-serif']/p",
                            )
                        )
                    )
                    abstract = article.find_element(
                        By.XPATH, ".//div[@class='abstract-section u-font-serif']/p"
                    ).text
                except:
                    abstract = "NA"
            ...
            articles = driver.find_elements(
                By.XPATH, "//li[@class='ResultItem col-xs-24 push-m']"
            )
